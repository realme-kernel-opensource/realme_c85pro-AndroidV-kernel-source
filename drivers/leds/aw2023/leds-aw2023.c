...
static int aw2023_led_change_mode(struct aw2023_led *led, enum AW2023_LED_MODE mode)
{
	...
	switch(mode) {
		...
		case AW2023_LED_INDIVIDUAL_CTL_BREATH:
			...
			led->pdata->rise_time_ms = 6;
			led->pdata->fall_time_ms = 6;
			led->pdata->led_mode = AW2023_LED_INDIVIDUAL_CTL_BREATH_MASK;
			break;
...
static void aw2023_brightness(struct aw2023_led *led)
{
	...
	if(led->pdata->led_mode == AW2023_LED_INDIVIDUAL_CTL_BREATH_MASK) {
		AW2023_DEBUG("enter AW2023_LED_INDIVIDUAL_CTL_BREATH_MASK\n");
		/*
		for (i = 0; i < 3; i++) {
			rgb_color[i] = led->pdata->color[i];
			AW2023_DEBUG("set color[%d] = %d\n", i, rgb_color[i]);
		}
		*/
		/* aw2023 led breath time tr1 & ton */
		for(i = 0; i < LED_MAX_NUM; i++) {
			...
			aw2023_write(led, AW2023_REG_LCFG0 + i, led->pdata->led_mode | rgb_color[i]);
		}
		...
		AW2023_DEBUG("%s: brightness[0] = %x brightness[1] = %x brightness[2] = %x\n", __func__, rgb_color[0], rgb_color[1], rgb_color[2]);
	}
// || led->pdata->led_mode == AW2023_LED_MANUAL_MODE_MASK
	if(led->pdata->led_mode == AW2023_LED_NEW_ALWAYSON) {
		AW2023_DEBUG("enter AW2023_LED_NEW_ALWAYSON\n");
		/* aw2023 led breath time tr1 & ton */
		for(i = 0; i < LED_MAX_NUM; i++) {
			aw2023_write(led, AW2023_REG_LED0T0 + i*LED_MAX_NUM,
				(led->pdata->rise_time_ms << AW2023_LED_TIME_SHIFT_MASK | led->pdata->hold_time_ms));
	...
	if((led->pdata->led_mode == AW2023_LED_BREATH_MODE_MASK) || TIMER_MODE || (led->pdata->led_mode == AW2023_LED_MANUAL_MODE_MASK)) {
		AW2023_DEBUG("enter AW2023_LED_BREATH_MODE_MASK\n");
		/* aw2023 led breath time tr1 & ton */
